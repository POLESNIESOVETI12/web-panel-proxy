...
echo "[4/5] Preparing safe Caddy route..."
cat > "$ROUTE" <<'EOF'
# Add the following line INSIDE the existing {$TPROXY_HOSTNAME} site block:
#
# import /etc/caddy/panel-route.caddy
#
handle_path /panel/* {
...
echo "  2. Inside the existing {$TPROXY_HOSTNAME} block add:"
echo
