...
VERSION="FINAL-IDEMPOTENT-CERT-6"
...
if [[ "$REUSE_EXISTING_HTTPS" == "1" ]]; then
    ...
    if [[ -z "$DNS_IP" ]]; then
        echo
        echo "DNS не установлен для ${DOMAIN}."
        echo "Повторите установку после добавления A-записи DNS."
        die "DNS CHECK FAILED"
    fi
    ...
    if [[ -n "$VPS_IP" && "$DNS_IP" != "$VPS_IP" ]]; then
        echo
        echo "DNS не установлен для этого VPS."
        echo "Повторите установку после исправления A-записи DNS."
        echo "DNS: $DNS_IP"
        echo "VPS: $VPS_IP"
        die "DNS CHECK FAILED"
    fi
