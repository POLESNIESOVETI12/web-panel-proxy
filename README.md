...
```bash
git add .
git commit -m "WEB PANEL PROXY V 2.0.1 public release"
git tag -a v2.0.1 -m "WEB PANEL PROXY V 2.0.1"
git push origin main
git push origin v2.0.1
```

Не перемещайте уже опубликованный тег `v2.0.1` на другой commit. Для следующего исправления используйте новый тег, например `v2.0.2`, и обновите `RELEASE_REF` в `install.sh` и `update.sh`.
